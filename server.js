...
app.get('/vehicles', authenticateToken, async (req, res) => {
    try {
        ...
        res.json(memoryStore.vehicles.length > 0 ? memoryStore.vehicles : [
            { _id: 'v_1', id: 'v_1', type: 'قلاب 20 طن', location: 'بغداد - الكرادة', dailyPrice: 150000, availableCount: 2 },
            { _id: 'v_2', id: 'v_2', type: 'لودر كوماتسو', location: 'البصرة', dailyPrice: 250000, availableCount: 1 }
        ]);
    }
});
...
app.get('/bids', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        res.json(withDriver.length > 0 ? withDriver : [
            { _id: 'bid_1', id: 'bid_1', tripId: tripId || 'trip_1', driverId: 'drv_1', driverName: 'محمد السائق', amount: 140000, status: 'pending', driver: { name: 'محمد السائق', rating: null, totalTrips: 0 } }
        ]);
    }
});
